...
| [Complete Tracks by Length](complete-tracks-by-length) | `one-to-one`, `many-to-one` | Generalizes [Complete Tracks](complete-tracks) to every track length: the accuracy (fraction fully correct) of tracklets or lineages that span N frames, for each length N. |
