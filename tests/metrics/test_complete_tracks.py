...
expected_results_one_to_one_single = [
    # (matched, (total, correct, complete_tracks))
...
def test_one_to_one_single(error_type, matched_func, expected_result):
    ...
    assert result.results["total_tracks"] == expected_result[0]
    assert result.results["correct_tracks"] == expected_result[1]
    if expected_result[2] is np.nan:
        assert result.results["complete_tracks"] is np.nan
    else:
        assert result.results["complete_tracks"] == expected_result[2]
...
@pytest.mark.parametrize("error_type", ["basic", "ctc"])
def test_larger_example(lineage, error_type, expected_result):
    larger_example = larger_example_1()
    complete_tracks = CompleteTracks(error_type=error_type, lineage=lineage)
