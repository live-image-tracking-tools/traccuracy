...
class SparseWeightedEdgeDivisionJaccard(Metric):
    ...
    Every value this metric returns is therefore either sparse-safe (the Jaccards and
    their counts, which only ever penalize evidence-backed errors) or agnostic (raw
    prediction-volume counts); see ``sparse_safe_keys``/``agnostic_keys`` below.
    ...
    **Combined score.** ``score = adj_edge_jaccard + division_weight * division_jaccard``
    ...
    # Jaccards and their counts only penalize evidence-backed errors, so they stay
    # valid on sparse GT; num_pred_nodes and total_node_ratio are raw prediction-volume
    ...
    agnostic_keys = frozenset({"num_pred_nodes", "total_node_ratio"})
